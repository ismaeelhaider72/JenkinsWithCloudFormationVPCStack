...
pipeline {
    ...
    stages {
        stage('Show Files') {
            environment {
              MY_FILES = sh(script: "withCredentials([string(credentialsId: 'AccessKeyID', variable: 'AWS_ACCESS_KEY_ID'), string(credentialsId: 'SecretAccessKey', variable: 'AWS_SECRET_ACCESS_KEY')]) {
